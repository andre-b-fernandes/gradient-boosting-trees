...
class CartRegressionTree:
    ...
    def fit(self, points: np.array, labels: np.array) -> None:
        def recursive_cart_tree(
            points: np.array, labels: np.array, level: int = 0
        ) -> Node:
            ...
            left = recursive_cart_tree(
                points=lhs_points, labels=lhs_labels, level=level + 1
            )
            right = recursive_cart_tree(
                points=rhs_points, labels=rhs_labels, level=level + 1
            )
